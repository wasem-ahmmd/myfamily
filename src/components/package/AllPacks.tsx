...
// import { fetchAllPackage } from "@/routesApi";
// import { useQuery } from "@tanstack/react-query";
import { useAllPackages } from "@/hooks";
...
const AllPacks = () => {
  // const { Plans,isError } = useAllPackages();
  // console.log(Plans);
  ...
  return (
    <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3 h-auto overflow-y-scroll p-6 justify-center justify-items-center content-start border-2 border-gray-300">
      {packageData?.map((item,index) => (
        ...
        >
          <div className="h-full overflow-hidden text-gray-900 border-2 border-gray-700 hover:border-gray-950 rounded-lg shadow-lg shadow-gray-900">
            {/* <div
              className={`${
                item?.country_details?.length > 1 ? "grid grid-cols-4 gap-1 ml-5 w-[270px] h-[180px]" : ""
              }`}
            > */}
              {/* {item?.country_details?.map((country: any) => ( */}
                <Image
                  ...
                  height={80}
                  priority
                  className={`object-cover items-center  w-[270px] h-[180px]  `}
                  //   ${
                  //   item.country_details.length > 1
                  //     ? "w-[70px] h-[80px]"
                  //     : "w-[270px] h-[180px] "
                  // }
            ...
            <div className="p-4  bg-BG-1">
              <div className="mb-3 flex flex-1 items-center justify-between gap-2 xxs:gap-0">
                <h2 className="font-extrabold italic font-sans text-lg">
                  {/* {item?.country_details?.map((name: any)=>  */}
                {item.country}
                {/* } */}
                </h2>
                <p
